...
{taxize} allows users to search over many taxonomic data sources for species names (scientific and common) to resolve synonymy. The `gnr_resolve()` function matches your supplied list with XX, XX, XX and scores from 0 - 1 to illustrate how well matched your name is to these soources. 
...
# Resolve names
temp <- gnr_resolve(unique(afd$VALID_NAME)[1:5], resolve_once = TRUE) 
temp |> print(n = 100)
