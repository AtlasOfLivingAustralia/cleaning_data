...
# Determine if COMPLETE_NAME == SYNONYMS
# If match is TRUE or NA then not a synonym
names |> 
  mutate(match = COMPLETE_NAME == SYNONYMS) |> 
  pull(match) |> 
  tabyl()

# Break up SYNONYMS column
split_names <- names |> group_split(VALID_NAME)

# As an example
# Make SYNONYMS long data
long_syn <- split_names |>
  pluck(46) |> 
  pull(SYNONYMS) |> 
  str_split(pattern = ";") |> 
  pluck(1) |> 
...
x <- split_names |>
  ...
  pluck(1) |> 
  str_trim() |> 
  str_match(pattern = regex("(?s)^(\\S+(?:\\s+\\S+)?)?(?:\\s+(.+))?"))

long_syn_split <- long_syn |> 
  mutate(SYN_NAME = x[,2],
         SYN_AUTHORITY = x[,3])

# Bind it with original names
split_names |>
  pluck(46) |> 
  select(-SYNONYMS) |> 
  bind_cols(long_syn_split)
