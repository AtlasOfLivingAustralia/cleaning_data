...
```
1. This line uses shorthand to write a function to append a suffix to a column name. An equivalent way of writing this is:  `function(column_name) {paste0(column_name, "_ala)}`. This is applied to each column name from `kingdom` to `species` in the `names_matches_ala` dataframe.
...
We can check whether our new rank names match what was already in our data set to see if our matched names were correct. We'll use  `filter()` to only return rows where `family_ala` does not equal `family`.
...
```{r}
#| eval: false
# takes a while to load, but this is the original query
gbif_commersonia_list <- galah_call() |>
  identify("commersonia") |>
  atlas_species()
```
...
gbif_species_list <- arrow::read_parquet(
  here::here("data", "lists", "gbif_commersonia")) |>
  collect()

# oh no
gbif_species_list |>
  distinct(species) |> print(n = 28)
```
