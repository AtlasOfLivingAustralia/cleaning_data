...
This chapter is a collection of the data used in this book. Most of the data is downloaded from the [Atlas of Living Australia (ALA)]() using the [galah package](), the ALA's R interface to downloading biodiversity data. You may download all the data throughout the book before you start, or find the relevant data set for a specific chapter.

All examples in this book are reproducible if run using data in this chapter. Data in this chapter is downloaded using data DOIs. These download the exact data used when the book was rendered on xxxxxxdate. Using these data should allow you to match the output of the examples over this book.
