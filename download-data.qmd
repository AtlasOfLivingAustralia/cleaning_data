...
In order to access data from the Spain GBIF node, we will need to reconfigure `galah` so that our query points to Australia. After that, we will download all records for the Pink Robin.
...
Open access biodiversity data come from many different providers such as universities, research institutes (museums and herbariums), government and the general public. As such, the data type and quality can vary considerably. For example, museums tend to supply records that are associated with a preserved specimen, whereas citizen sourced data are often images captured from a smart phone. Generally, biodiversity data from reputable providers tend to be more reliable compared to crowd sourced data as some organisms can misidentified. 

If your project requires research grade data, we recommend refining your query by: 

- year
- basis of record
- assertions 

Refining your download query ensures higher quality data and also reduces the download size as many data infrastructures impose constraints to download size. Below we have illustrated how to do so in `galah`. 
