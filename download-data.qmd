...
```{r}
galah_config(email = "firstname.surname@email.com",
             username = "user.name",
...
write_parquet(pink_robin, "data/galah/pink_robin")
```
...
Open access biodiversity data come from many different sources, 

Additionally, it is typical that data infrastructures impose constraints to query size.

We suggest for broad data scopes that it may be more efficient to further refine your query before downloading. Users may want to narrow their query by *basis of record* or by a *year filter* 
