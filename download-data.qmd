...
**Naming authorities** are the people or organisations that create updated or revised lists of species, and where they fit on the taxonomic tree. Species names and taxonomic groups in these lists can vary when there are disagreements about what distinguishes a new genus, species, subspecies etc. Taxonomy is hard!

Some data (especially from open source repositories) can contain data with conflicting taxonomies from different naming authorities. Different taxonomies can cause unexpected errors later in an analysis if you are unaware of which naming authorities were used in your data.
...
In the Atlas of Living Australia, the [Australian Plant Name Index (APNI)](https://www.anbg.gov.au/apni/) is the primary naming authority for plants. With the [Australian Faunal Directory (AFD)](https://biodiversity.org.au/afd/home) the main taxonomic catalog for animal species. For a full list, see the [appendix](#appendixNamingAuthorities).

Data that doesn't match your preferred naming authority or contains data from multiple naming authorities may need to be matched to a single taxonomic backbone <!-- might be worth showing how in an appendix-->.
...
orchids |> 
  filter(
    ...
    !scientificName %in% raw_scientificName
    ) |>
  select(scientificName, raw_scientificName) |>
  slice(30:50)
...
<!-- Dax note: I got to here with structuring the sections. Feel free to disagree and move things around -->


## Refining your data download

<!-- Dax note: I have moved some of the below info (basically everything in By Year) into the previous chapter. Not sure what to do about Basis of Record, though. Perhaps we can rework this into the Pre-cleaning chapter? -->
...
<!-- Fonti/Margot suggestion. Dax thinks that this could be a nice idea, even if it was just a tiny example of how to do fuzzy matching, or some resources to help people get started with this-->
