...
=======
<!-- SB: I tried to provide a bit more context than what previously existed, but it's possible I got too waffly. Will not be offended if you cut everything** -->

Biodiversity datasets often contain missing values. Analysis and visualisation software can have different behaviours in response to this: they might drop the missing values, provide a warning and process anyway, or, return an error. Understanding the nature of missing data can also be useful for informing how you proceed with analysis. 

For instance, understanding how much missing data there is overall and whether it is random or consistent can influence what you choose to do with it (e.g. remove vs. impute), which in turn, can affect the results of statistical models based on those datasets. Understanding where missing data occur in a dataset is therefore an integral part of exploratory data analysis. This chapter will show you how to detect and remove missing values.     
>>>>>>> Stashed changes
...
=======
The `summary()` function (base r) provides summary statistics for each column in your table. The output includes the number of missing values in each column (`NA's`). 

::: {.callout-important title="`summary()` NA's"}
From this, we can tell that there are missing values in the columns: `decimalLatitude`, `decimalLongitude`, `eventDate`, and `month`.  
:::
>>>>>>> Stashed changes
...
The `skimr` package provides a fast and simple way to identify columns with missing values and quantify the level of missing data using the `skim()` function.
>>>>>>> Stashed changes
...
=======
::: {.callout-important title="`skimr()` NA's"}
The output shows that there are 7 columns with missing values (under `skim_variable`), along with information about the number of missing values in `n_missing` and the proportion of non-missing values in `complete_rate`.
:::
...
Or we can specify the column for retrieval of particular `NA`'s (in this case column `cl22`).
...
::: {.callout-important title="Excluded records"}

153 records in our dataset had no spatial coordinate information and were excluded.
:::
...
<!-- I think this footnote is kinda important to understand why there are NA's in the new colums so im thinking it would be better off not as a footnote especially because the article is about NA's explicitly it might raise some red flags OT -->
...
```
 ...
 <!-- need another piccy here but the images portal is down sorry OT --> 
