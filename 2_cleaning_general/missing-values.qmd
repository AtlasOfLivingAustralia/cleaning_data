...
Biodiversity datasets often contain missing values, and analysis and visualisation software can have different default behaviours in response to this: they might drop missing values, provide a warning, or return an error. Understanding the nature of missing data can also be useful for informing how you proceed with analysis. For instance, understanding whether missing data are random can influence what you choose to do with it (e.g. remove vs. impute), which in turn can affect the results of statistical models based on those datasets. Understanding where missing data occur in a dataset is therefore an important part of exploratory data analysis. This chapter will show you how to detect and remove missing values.     
...
The `summary()` function does exactly that - it provides summary statistics for each column in your table. The no-frills output includes the number of missing values in each column (`NA's`). Here, we can tell that there are missing values in the columns: `decimalLatitude`, `decimalLongitude`, `eventDate`, and `month`.  
...
The skimr package provides a fast and simple way to identify columns with missing values and quantify the level of missing data.   
...
The visdat package contains functions to visualise different aspects of dataframes. `vis_miss()` allows users to visualise the extent and location of missing data throughout a dataframe, with additional arguments for customising the visual output through clustering or sorting the missing data.  
