...
Each column in a table contains data of a specific **class**. Classes are the "type" of data in a column and determine how those data are interpreted in R. They impact how we can modify those data or work with them.

For instance, it doesn't make sense to apply a mathematical equation to a word or a sentence. The phrase "hello" doesn't reveal whether something is true or false. Knowing what *types* of data are in each column of your table will ensure those data behave as expected later on. 

You can convert data types through different commands in R if they have been incorrectly interpreted or allocated, but generally, functions will not work if the data type is not compatible. 

Depending on the source of your dataset column names in a table may be uninformative (e.g. `col1`, `tga42.D`). They might also be oddly formatted once imported into R (e.g. `How.Much.Soil.Is.In.This.Plot..`), or, internally inconsistent (e.g. `species_name`, `scientificName`). Modifying these can make it much easier to work with the data. 


### Prerequisites

In this chapter, we will use *Litoria* frog occurrence data since 2020 in Tasmania. Here is how we retrieve that data using [galah](https://galah.ala.org.au/). 
...
If you are using a `tibble`, the class is also displayed below each column name when you view your table (here there are two classes of data: character <chr>,  double <dbl> and date-time <dttm>). 
...
From these quick overviews of the data, we've learned:
  ...
  *  Column `scientificName` are strings of text (type `character`)
...
Here, the column classes are what we'd expect given the types of data in each column. 
...
One of the most useful column name cleaning functions is `clean_names()` from the [janitor](https://cran.r-project.org/web/packages/janitor/vignettes/janitor.html) package. This function will make all of your column names consistent, based on your preferred naming convention (defaults to snake case). 
...
`rename_with()` is a more powerful version of `rename()`. It allows more advanced renaming by using functions to rename matching columns.
...
frogs |>
  rename_with( ~ tolower(gsub("decimal", "new_", .x, fixed = TRUE))) |>
  dplyr::select("new_latitude", "new_longitude") |>
rmarkdown::paged_table()
```
...
```{r}
#| echo: false
frogs_separate |> rmarkdown::paged_table()
```
