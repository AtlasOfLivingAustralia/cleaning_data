...
From these quick overviews of the data, we've learnt:

  *  Columns like `scientificName` are strings of text of type `character` 
  *  Columns like `decimalLatitude` and `decimalLongitude` are numbers with decimal points of type `double`
...
Here, the column classes are what we'd expect, given the types of data in each column, but this is not always the case. For instance, changing just one of the values in `decimalLatitude` to a "degrees minutes seconds" format will cause the entire column's class to be changed to `character` to prevent loss of data.
...
Column names should use consistent naming conventions. Common naming formats include "snake case" and "camel case", which are popular because they are both human- and machine-readable. 
...
R is case sensitive, so two names with the same letters but different capitalisation are considered different names (e.g. event vs. Event). Using a consistent naming convention, like camel case or snake case, makes it easier to avoid these errors. Snake case is more popular in R, and is the naming convention we recommend. Data downloaded from the ALA is in camel case[^column-names-and-classes-1].
...
One of the most useful column name cleaning functions is `clean_names()` from the janitor package. This function will make all your column names consistent, based on your preferred naming convention (defaults to snake case). 
