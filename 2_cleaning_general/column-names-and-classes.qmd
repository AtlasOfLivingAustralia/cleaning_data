...
For instance, it doesn't make sense to apply a mathematical equation to a word or a sentence. The phrase "hello" doesn't reveal whether something is true or false. Knowing what *types* of data are in each column of your table will ensure those data behave as expected later on. You can convert data types through different commands in R if they have been incorrectly interpreted or allocated, but generally, functions will not work if the data type is not compatible. 
...
If you are using a `tibble`, the class is also displayed below each column name when you view your table (here there are two classes of data: character <chr> and the double <dbl>). 
