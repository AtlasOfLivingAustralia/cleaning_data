...
across authorities about what taxonomy is true. In different countries,
...
-   A subset of invertebrate occurrence records taken from the [Curated
    ...
    set](https://data.csiro.au/collection/csiro:56679). Download the `inverts` dataset from the [Data in this book chapter](../data-in-this-book.html#inverts).
...
inverts <- here::here(\"path\", \"to\", \"inverts.parquet\") |>
  arrow::open_dataset() |> 
  collect()
...
a)  Records have been identified to different taxonomic ranks (we can
...
identify which species are relevant. Some common examples are using a
lists of introduced, invasive, threatened or sensitive species to
identify species records of interest.

There are several ways to filter records to those that match names on a
species list. Let's see how to match a species list accessed using
galah, or an external species list loaded into R.
...
here is a species list of *Eucalyptus* downloaded from GBIF.
...
marine_sp <- read_csv(here::here("worms.csv")) # <1>

marine_sp |>
  filter(scientificname == "Lupocyclus inaequalis") |>
  select(AphiaID, scientificname, status)
```
1. This is a subset of the WoRMS dataset. You can download it from the [Data in this book chapter](../4_appendices/data-in-this-book.html#marine_sp)
