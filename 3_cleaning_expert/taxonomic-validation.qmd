...
An an example, the ALA uses `"subsp."` to designate subspecies of Acacia
...
<!-- I found this example confusing. The previous example discusses how the same taxonomic rank (subspecies) is displayed differently across taxonomic groups in the same field (scientificName), which seems like a good thing to highlight. But this example illustrates how different fields (scientificName, species) contain different information, which seems like something you would expect (or why even have two different fields?). I think it's worth highlighting if, similarly to subspecies, subgenera are displayed differently across taxa within scientificName. Otherwise I would get rid of this example. The ALA does do this very odd thing with subgenera where they're included in scientificame, but often left blank in the subgenus field. Probably not something we need to highlight here though -SB  -->

Subgenera are present in many Animalian clades, but their formatting can vary. 
In the ALA’s `scientificName` field, subgenera are specified in brackets between
...
<!-- Given that this can now be downloaded from the website again as a simplified checklist i.e. single csv file, should we name the file something a bit more sensible than "GRIIS_Australia_20230331-121730.csv"? Probably also worth downloading the latest version because there have been some significant changes to the contents of the list in the last year or so -SB  -->

```{r}
#| eval: false
griis <- read_csv(here("griis_australia_20240712-155356.csv")) # <1>

glimpse(griis)
```
1. We renamed the downloaded file from `record 20240712-155356.csv` to `griis_australia_20240712-155356.csv`
...
griis <- read_csv(here("data", 
                       "lists",
                       "griis",
                       "griis_australia_20240712-155356.csv"))
glimpse(griis)
```
...
<!-- Text says legless lizards but code says eucalypts -SB  -->
...
galah. We'll save the results in `names_matches_ala`.
...
<!-- This would contain subspecies though, I think, so probably fewer than 373 species? -SB  -->
...
<!-- I find this example a bit confusing because I'm not sure what I'm supposed to take away from it. From looking at the output for a minute or two, I don't find it entirely intuitive (and I'm also confused about some things which are unrelated to what's being demonstrated here e.g. there are rows where acceptedScientificName contains subsp. but the value in taxonRank is SPECIES). I think it might be worth spelling out in more detail what exactly we're expecting readers to take away from this - SB -->
...
<!-- A note to check whether this still times out - SB -->
