...
*CW: One of the simplest ways to determine whether there are any immediate issues with taxonomic names is to print some of them. Most biodiversity datasets have a `scientificName` or `scientific_name` field that specifies the lowest identifiable scientific name for each record. Looking at it here, we can already notice some characteristics of the names in our data: a) records have been identified to different taxonomic ranks (we can see subspecies, species, genus and family here), b) some names are formatted in all capitals while others are not, and c) some names have bracketed parts.*
...
*CW (maybe as another tab): Subgenera: As a second example, subgenera are present in many Animalian clades, but their formatting varies across taxonomic fields. In the ALA's `scientificName` field, subgenera are specified as a bracketed term between the genus and species, while they are not specified at all in the `species` field.*
...
Now let's join our matched names to our `inverts_sample` data. **CW: Ignore what I said about this right_join - it's correct so leave as is**
...
**CW: I think it would be good, if its concise enough, to explain that GBIF does have these names stored as synonyms which is why it matches it still (and lists in under scientific_name)**
...
*CW: Not sure on this, but it might be helpful to show a taxonomicRank column if its present?*
