...
**CW: I think it would be good, if its concise enough, to explain that GBIF does have these names stored as synonyms which is why it matches it still (and lists in under scientific_name)** <!-- I think I have addressed this - DK -->
...
In the above examples, taxonomic searches match correctly on GBIF because GBIF uses a special, massive database of accepted and superseded names and synonyms. This massive database allows GBIF to match *lots* of different names. ALA, on the other hand, uses a much smaller taxonomic names database that matches to its taxonomic backbone. THis database is easier to store but less complete than GBIF's.
