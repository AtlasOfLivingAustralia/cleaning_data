...
*CW: In all spatial datasets, the most important columns are `decimalLatitude` and `decimalLongitude` (or similarly named columns). These contain the latitude and longitude of each observation in decimal (rather than degrees) form.*

*CW: Added colour to points on map*
...
ggplot() + 
  ...
  geom_sf(data = banksia_sf, colour = "orchid")
```
...
*CW: The methods in this section are used above in the quick visualisation code*
...
**CW: This needs to be specified clearly that the 10m uncertainty is the default*
...
*CW: I've had a look at some species with data generalizations and I think quokkas (Setonix brachyurus) could be good. Could then frame this section as what to look for to indicate obscuring of data (unnatural gridded distributions as shown in this map) but that this doesn't instantly mean obscuring, as it could be survey locations for example)*
...
works the same as an `ifelse` statement (but can handle many of statements at once). The first updates our `decimalLongitude` column so that when `decimalLongitude` is less than 0, we remove the negative symbol, otherwise we keep the original longitude value. The second updates our `decimalLatitude` column so that when `decimalLatitude` is greater than 0, we'll "glue" on a negative symbol, otherwise we keep the original latitude value.

*CW: Probably best just to fix the wrong signs by multiplying by -1*
...
<!-- This example is a more complicated example. Does it still work or should we pick a simpler example? -->
*CW: I think this is a great example but might be better placed as a case study at the end of the chapter - could link to it here though*
