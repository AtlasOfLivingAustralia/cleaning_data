...
#| echo: false
library(arrow)
pardalotes <- open_dataset("data/pardalotes", format = "csv")
```

```{r}
#| eval: false
library(arrow)
pardalotes <- open_dataset("tmp/pardalotes", format = "csv")
```
...
#| eval: false
library(readr)
pardalotes <- read_csv("tmp/pardalotes")
```
:::
