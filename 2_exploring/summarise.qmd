...
Now we need to count the number of birds per day. We can do this by using dplyr's `group_by()` and `summarise()` functions. 
...
```{r}
#| fig-cap: "Bird record counts per day (2022)"
...
You can use the same method above to group by week as well. 
