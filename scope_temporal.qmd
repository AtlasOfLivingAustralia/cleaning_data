...
library(viridis)
library(rmapshaper)
# TODO maybe move down, or split up to only used packages for each section so
...
the `galah` package to query the Atlas of Living Australia, which is the largest
biodiversity data aggregator in Australia.

The `atlas_counts()` function returns a count of records. By itself, it returns
a count of all records in the Atlas of Living Australia.
...
We can narrow this query using `galah` helper functions. In this case, we narrow
