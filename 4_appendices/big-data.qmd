...
DuckDB is a database management system that can read, write, and manipulate larger-than-memory datasets. This can be very useful for working with large relational tables, for instance, or appending large amounts of data to existing datasets. To do this programmatically in R, without having to use SQL, we can use dbplyr together with duckdb to perform a similar operation to the one we did using arrow above. Here, we write the pardalotes dataset to an in-memory database and then summarise it. 
...
One of the great really cool things about duckdb and arrow (you probs want to rephrase this a bit more professionally) is zero-copy integration, which means you can pass datasets back and forth between the two engines within an analysis pipeline, with very little loss of efficiency. So if there is some functionality that is not supported by one of the packages, but is by the other, you can simply switch in the middle of an analysis. The duckdb and arrow blogs have written about this in greater detail [here](https://arrow.apache.org/blog/2021/12/03/arrow-duckdb/) and [here](https://duckdb.org/2021/12/03/duck-arrow.html). 
