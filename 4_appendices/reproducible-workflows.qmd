...
One of the strengths of using code for data cleaning, analysis and visualisation is that code *should* be easier to share and use to reproduce results than point-and-click software. However, research finds that code isn't shared very often in ecological and evolutionary journals [@kimmel_empirical_2023], and when it is shared, a surprisingly small proportion of coding workflows run from start to finish [@culina_low_2020; @roche_public_2015].

This chapter details how to improve the reproducibility of your R code workflows, originally described in [this Nature Eco Evo blog](https://communities.springernature.com/posts/improving-code-reproducibility-small-steps-with-big-impacts).
...
Let's say a scientific paper claims that a species of frog has an 80% probability of shifting its distribution southward by 60 kilometres. Without being able to reproduce the result (or the data on which that result is based on), it's difficult to know how trustworthy this statistic really is or the extent that this result should affect our understanding of this species. What factors are influencing this statistic? What is the sample population that this statistic was derived from? What data cleaning steps were made prior to analysis? 

A reproducible workflow---from raw data to analysis to the the final result---allows me to at least attempt to answer some of these questions!

:::

Data cleaning is an essential step of an analytic workflow. Without the ability to reproduce the cleaned dataset used in an analysis, often an entire analysis might not be able to be run again, much less reproduce an individual result!  


## Reproducible environments

To allow coding scripts to run again, it's important to be able to reproduce the environment that the original code was able to successfully run in. In R, this usually involves setting your working directory correctly so that files and packages can be located and loaded[^wd]. Below are several ways to make working environments better for sharing.

[^wd]: A common way to first learn to set a working directory in R is clicking on Session --> Set Working Directory, or using `setwd()`. Both of these methods are poor practice for reproducibility because they require users to manually enter paths that are prone to break when the script is opened on another computer or in a new location.

### R projects

[R projects](https://r4ds.hadley.nz/workflow-scripts#projects) are the best way to set project directories when using R Studio. `.Rproj` files tell R Studio what the top directory is for a project. Opening an R project (either in R Studio or by opening the `Rproj` file) automatically opens the project directory and sets the working directory. All file paths will stem from that top directory while using that R project. 
