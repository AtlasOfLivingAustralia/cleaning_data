...
They contain values of a variable (like temperature) for each pixel of a grid, and
Each pixel of the grid represents a square area (e.g., 1 km^2^). Just like
...
```
1. Many shapefiles are a large file size. `ms_simplify()` from the rmapshaper package simplifies the shapefile by reducing the number of points that make up the polygon while maintaining its overall shape. This is a handy way to reduce the size of your shapefile in R.
...
We discuss these considerations in more detail in the [Taxonomic Validation chapter](../3_cleaning-expert/taxonomic_validation.qmd).
...
edit & filter download queries. Additionally, galah can access data from 10 other
Living Atlases and GBIF.
...
[VertNet](https://github.com/ropensci/rvertnet). spocc is particularly useful because it allows for a single download request in R to query and return data from multiple data sources in a single nested dataframe.
