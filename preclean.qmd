...
If you're using data from particular institutions it's worth checking the metadata for the entire dataset to delineate if there is anything you should know about the data. You can do this by googling the dataset, or searching the dataset/institution on the ALA. The metadata on the ALA is submitted with the data, it's not always of high standard which is why it's worth investigating externally, an example of some good metadata is [FrogID from the Australian Museum](https://collections.ala.org.au/public/show/dr14760). A benefit to reading metadata is understanding the limitations of your dataset. FrogID's metadata for their dataset has been publishes as a paper [@rowley2020frogid], and from reading through it you'll find:
