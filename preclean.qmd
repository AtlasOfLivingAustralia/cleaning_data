...
String inconsistencies include mispellings, capitalisation errors, misplaced punctuations or trailing white spaces. We will use the `janitor` R package to explore whether our data has any of these issues. The function `tabyl` will compute a counts and percentage of total rows for each unique value. 

We recommend `tabyl-ing` character strings that are relevant to your project. For example, here is the [`stateProvince`](https://dwc.tdwg.org/terms/#dwc:stateProvince) in alphabetical order. 
...
(I wonder if this is really the place for this or better to just do this in their respective Taxonomy and Spatial chapter)

Missing values are ubiquitous in biodiversity data and need to be carefully explored before deciding their fate. In this book, we will on focus on missing data in taxonomic and coordinate date. 

In most cases, missing taxonomic information such as higher order taxonomic classifications can be back-filled from a naming authority of your choice.
...
qmplot(decimalLongitude, decimalLatitude, data = african_ele_2, maptype = "toner-lite")
```
