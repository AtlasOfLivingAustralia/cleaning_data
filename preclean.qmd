...
One shortfall to the `skimr` output is that it doesn't provide ranges for numeric variables which we can achieve with the `summary()` function in conjunction with functions from the `dplyr` package

```{r, eval = TRUE}
library(dplyr)

# Isolate numeric columns with `select` and `where` then call `summary` function
african_ele %>% 
