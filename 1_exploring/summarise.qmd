...
In the last chapter, we learned how to see a brief glimpse of our data's structure, including how many rows there are, what columns there are, and where there might be missing data. In this chapter, we learn how to briefly summarise ecological data. These summaries will fall within three domains: **Taxonomic**, **spatial** and **temporal**. Summarising our data will help us see the diversity in our dataset, learn more about its scope, and determine whether it is the right dataset for our analysis.

Over this chapter, there are many places where we will use the [galah](https://galah.ala.org.au/) package to summarise our data. The [galah](https://galah.ala.org.au/) package can summarise data on the server-side prior to downloading any data locally onto your computer meaning that you can filter or summarise data without needing to have them on your local computer first.

We will show how to use [galah](https://galah.ala.org.au/) (*prior* to download) or [dplyr](https://dplyr.tidyverse.org/articles/dplyr.html) (*after* a download) when both options are available.
...
Prior to downloading occurrences, it can be useful to see a taxonomic breakdown of what occurrence records exist for our download query. To see how many total occurrence records there are for *Alcendinidae*, we can count the total number of records...
...
...or group by a taxonomic clade like genus...
...
We can use maps to visually summarise our data. To do so, we will use the [sf package](https://r-spatial.github.io/sf/) to handle spatial data, and the [ozmaps package](https://mdsumner.github.io/ozmaps/) to get maps of Australia (as [vector data](../4_appendices/where-to-get-data.qmd#spatial-data)).
...
Our first step is to get a map of Australia, and an excellent place to get one is from the ozmaps package. We will transform its Coordinate Reference System (CRS)[^summarise-1] projection to EPSG:4326 to match the CRS projection of ALA data[^summarise-2].
