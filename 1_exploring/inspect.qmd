...
yourself with the data. In this chapter we will cover ways to initially inspect both the
data and metadata of a dataset.

### Prerequisites

In this chapter we will use Kinfisher (*Alcedinidae*) occurrence records in 2023 from the ALA.
...
doi_birds <- "https://doi.org/10.26197/ala.55960665-74c2-4d1f-81ad-ba4cdd2269fa"
```
...
doi_table <- open_dataset(here::here("data", "galah-doi", "doi_table")) |>
  filter(chapter == "Inspect") |>
  collect()

doi_birds <- doi_table |> filter(name == "birds") |> pull(name)
...
**Note:** You don't need to run this code block to read this chapter. This code downloads the latest data from the ALA (which won't exactly reproduce results in this chapter).
