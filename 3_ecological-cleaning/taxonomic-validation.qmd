...
Let's say we want to compare these observations to data retrieved outside of the ALA and decide that we'd prefer to use GBIF's[^taxonomic-validation-1] taxonomy. ALA data uses its own taxonomic backbone that differs to GBIF's (depending on the taxonomic group), so we will need to amend our taxonomic names to match GBIF's.
...
Homonyms are identical names that are used to refer to different taxa. For example, the name *Morganella* is a [genus of bacteria](https://bie.ala.org.au/species/NZOR-6-116469), a [genus of fungi](https://bie.ala.org.au/species/https://id.biodiversity.org.au/node/fungi/60091999#overview), a [genus of scale insect](https://bie.ala.org.au/species/https://biodiversity.org.au/afd/taxa/3d1bad6b-4753-430c-a009-c6f4787b3421), and a [genus of brachiopod](https://en.wikipedia.org/wiki/List_of_brachiopod_genera) from the Devonian period[^taxonomic-validation-5]!

[^taxonomic-validation-5]: Referred to as "[the Age of Fishes](https://www.nps.gov/articles/000/devonian-period.htm#)", the Devonian Period occurred \~419 to \~359 million years ago.
...
#### taxize not working
As of 17 September, 2025 the taxize functions in this section hit `HTTP 404` errors. We suggest finding other solutions until these issues are fixed.
:::
...
# Resolve names
resolved <- gnr_resolve(sci = "litoria caerulea", best_match_only = TRUE)
resolved
```
