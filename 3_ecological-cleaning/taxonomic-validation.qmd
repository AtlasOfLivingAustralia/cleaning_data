...
-   A subset of invertebrate occurrence records taken from the [Curated Plant and Invertebrate Data for Bushfire Modelling data set](https://data.csiro.au/collection/csiro:56679), saved in the `inverts.parquet` file
...
::::::: aside
<img src="https://images-assets.ala.org.au/store/2/5/1/d/9525c6da-fd47-41fa-8eff-55c6747ed152/thumbnail_large" class="rounded"/></img>

::: figure-caption
[*Todiramphus (Todiramphus) sanctus* perched on a branch. Photo by Kerri-Lee Harris CC-BY-NC 4.0 (Int)](https://biocache.ala.org.au/occurrences/77b8aac0-18af-4ec6-a03c-ff825859a6f3)
:::

<img src="https://images-assets.ala.org.au/store/c/7/a/b/3544fca1-e201-4e90-8551-64e73ed6ba7c/thumbnail_large" class="rounded"/></img>

::: figure-caption
[*Paradelma orientalis* smiling at the camera.<br>Photo by dhfischer CC-BY-NC 4.0 (Int)](https://biocache.ala.org.au/occurrences/1ccd81c3-a642-4d78-a6a3-4393978e6a73)
:::

<img src="https://images-assets.ala.org.au/store/0/5/6/a/ca48dfdd-15b3-4ac5-a392-cf5f93a6a650/thumbnail_large" class="rounded"/></img>
...
<img src="https://images-assets.ala.org.au/store/3/0/2/e/1ebe2767-0f46-43ba-afe1-c49d4046e203/thumbnail_large" class="rounded"/></img>
...
1.  Records have been identified to different taxonomic ranks (family, genus, species, subspecies)
...
:::{.callout-warning}
# Broken in galah version 2.2.0
The `isAuthoritative` column is missing in the output of `search_all(lists)` in galah version 2.2.0. As a result, the following code chunk doesn't work as a filtering method for now.
:::
...
##### Lists & taxonomic names matching
...
If we wish to use this threatened species list to filter a query, we'll need to ensure we `group_by()` either `scientificName` or `taxonConceptID`. Both fields capture the lowest taxonomic identification, and will match a list with names at multiple taxonomic levels appropriately. For example, grouping by `taxonConceptID` will return taxonomic information of matching species *and* subspecies (see this [ALA Labs post](https://labs.ala.org.au/posts/2025-07-20_species-lists-in-R/#download-data) for more information).

<!-- 
Note: The chunk below seems to trigger a weird error sometimes
      The message is 'ERROR: [non-error-thrown] undefined' 
      The way to get around it is to run the code in your console up to at least line 347, then render again and it should work
-->
...
We can also use lists downloaded outside of galah to filter our data. As an example, let's filter our taxonomic names to include only Australian names from the [Global Register of Introduced and Invasive Species (GRIIS)](https://griis.org/download). After downloading this list and saving it in your working directory, we can read the list into R. Taxonomic names are stored in columns with an `accepted_name` prefix.
...
### When a name match returns a different result than expected
...
Sometimes occurrences are not recorded to the lowest taxonomic level, such as subspecies. There are many reasons why this might occur, and it often has more to do with the source of the record than the accuracy of identification. In the case of *Perameles gunnii gunnii*, there are 15 records at the subspecies level and over 3,000 records of *Perameles gunnii* in Tasmania. Most authorities have not recorded the subspecies simply because it is the *only* subspecies of *Perameles gunnii* found in Tasmania. In this situation, the best way to get records of *Perameles gunnii gunnii* would be to intersect a query for *Perameles gunnii* with a geospatial boundary of Tasmania. Depending on your taxon of interest, a query for subspecies may need to include both species and subspecies names, and vice versa in a query for species. 
...
Let's say we want to compare these observations to data retrieved outside of the ALA and decide that we'd prefer to use GBIF's[^taxonomic-validation-1] taxonomy. ALA data uses a taxonomic backbone based on the National Species List, maintained by the Australian Biological Resources Study, that differs from GBIF's, so we will need to amend our taxonomic names to match GBIF's.

[^taxonomic-validation-1]: [Global Biodiversity Infrastructure Facility (GBIF)](https://www.gbif.org/)
...
Now let's filter `gbif_species_list` to only "accepted" names[^taxonomic-validation-2] and select the same taxonomic names columns.

[^taxonomic-validation-2]: GBIF's species list is quite comprehensive, and it includes the `taxonomicStatus` of a name as "accepted", "synonym", "variety" or "doubtful". To keep our example simpler, we are only using the accepted names.
...
`matched_names` now contains the full taxonomy from the ALA and GBIF for all matched species[^taxonomic-validation-3].

[^taxonomic-validation-3]: Several species names did not match to GBIF. In a complete data cleaning workflow, these should be investigated as the ALA and GBIF might use synonym names to describe the same species or subspecies.
...
gbif_taxa |>
  select(canonical_name, phylum, class, order)
```
...
Now that we are aware of the differences between GBIF and ALA names, if we would like to use GBIF's taxonomic names, we can join the columns with the suffix `_gbif` to our `eucalypt` occurrences data, and then replace the old taxonomic names columns with the GBIF names columns[^taxonomic-validation-4].

[^taxonomic-validation-4]: There were some names that did not match GBIF, meaning their taxonomic columns contain `NA` values. Be sure to either fix these `NA` values before merging dataframes, or back-fill after merging dataframes. Otherwise, you might add missing data in your data set unintentionally!
...
#### Global vs. Australian taxonomy

Be aware that there are many areas of disagreement between global taxonomies and Australian taxonomy. A well known case was (until recently) that Australian tree frogs (hylids) were known by the genus name *Litoria* in Australia, but *Ranoidea* internationally. Searching for "Litoria" in GBIF would not previously return all Australian hylid records and the converse was true in the ALA.
...
gbif_taxa |> 
  select(canonical_name, genus, species)
```
...
:::: aside
<img src="https://images-assets.ala.org.au/store/b/7/5/6/ed83fa14-2ef0-4abd-a90b-fedc7cd1657b/thumbnail_large" class="rounded"/></img>
...
:::: aside
<img src="https://images-assets.ala.org.au/store/3/7/b/7/210de50e-d758-41f1-8f87-448343937b73/thumbnail_large" class="rounded"/></img>

::: figure-caption
[*Androcalva roseava rosea* flowering.<br>Photo by Will Cornwell CC-BY 4.0 (Int)](https://biocache.ala.org.au/occurrences/4f3def8a-8f02-40fe-9ce3-b08663a5d899)
:::
::::
:::::::
...
### Checking for synonyms {#sec-synonyms}
...
For Australian data more specifically, [The National Species List](https://biodiversity.org.au/nsl/) is the accepted national standard for checking scientific names and the recommended place to start. If you are working with plant data, the [APCalign](https://traitecoevo.github.io/APCalign/index.html) package offers tools to align and update Australian plant taxon names (see the [Packages section](#sec-packages) below for more detail).


## Detecting homonyms

Homonyms are identical names that are used to refer to different taxa. For example, the name *Morganella* is a [genus of bacteria](https://bie.ala.org.au/species/NZOR-6-116469), a [genus of fungi](https://bie.ala.org.au/species/https://id.biodiversity.org.au/node/fungi/60091999#overview), a [genus of scale insect](https://bie.ala.org.au/species/https://biodiversity.org.au/afd/taxa/3d1bad6b-4753-430c-a009-c6f4787b3421), and a [genus of fossil brachiopod](https://en.wikipedia.org/wiki/List_of_brachiopod_genera) from the Devonian period[^taxonomic-validation-6]!

[^taxonomic-validation-6]: Referred to as "[the Age of Fishes](https://www.nps.gov/articles/000/devonian-period.htm#)", the Devonian Period occurred \~419 to \~359 million years ago.

:::{.callout-note collapse="true" appearance="minimal"}

#### Capitalisation standards

If you're cleaning strings with a method that is case sensitive, it is important to be aware of capitalisation standards for scientific names. For example, the first letter of genus should always be capitalised and the species name (and any subspecies, varieties etc.) should always be lower case (e.g. *Homo sapiens*). Not using the correct case increases the likelihood of encountering a homonym error. The only time this is not true is if you are using an undescribed species name such as *Anilios* sp. ‘Cape Range’ or *Grevillea* sp. Victoria Desert (R.Davis et al. RD 11611) WA Herbarium*. In these cases it is essential to get the capitalisation and spacing identical (note, for example the absence of a space between “R.” and “Davis”, which is a standard used by all herbaria in Australia).
...
The [taxize package](https://github.com/ropensci/taxize) allows users to search across many taxonomic data sources for hierarchical taxonomic information, such as species names (scientific and common), to resolve synonyms and homonyms.
...
```{r}
# Retrieve synonyms
tsn <- get_tsn(resolved$matchedCanonicalSimple) # works as of 2025-10-03
synonyms(tsn) 
```
...
The [worrms package](https://docs.ropensci.org/worrms/) is the R interface to the World Register of Marine Species (WoRMS). When working with data from this database, the worrms R package has the ability to cross-check synonyms in their database using their taxonomic ID (`AphiaID`).
...
::: aside
Download the `worms.csv` file from the [Data in this book chapter](../data-in-this-book.html#marine_sp).
:::
...
The [APCalign](https://traitecoevo.github.io/APCalign/index.html) package uses the [Australian Plant Census (APC)](https://biodiversity.org.au/nsl/services/search/taxonomy) and [Australian Plant Name Index (APNI)](https://biodiversity.org.au/nsl/services/search/names) to help users update species lists and match them to their established status (native/introduced) within different states/territories. Using our *Eucalyptus leucoxylon* example, let's return the aligned, accepted and suggested name of the first 5 accepted scientific names in our `e_leucoxylon_names`.
...
Notice that the results are slightly different to those in `gbif_species_list` example that that we saw above in the [Synonyms section](#sec-synonyms). Specifically, several synonyms are categorised as accepted subspecies names, which more accurately aligns to the APC's accepted taxonomic names.

APCalign provides additional functions like [`align_taxa()`](https://traitecoevo.github.io/APCalign/reference/align_taxa.html) and [`update_taxonomy()`](https://traitecoevo.github.io/APCalign/reference/update_taxonomy.html) to help update larger lists of taxonomic names and provide explanations about how names were matched.
...
aligned_taxa |>
  rmarkdown::paged_table() # paged table format
```
...
The best first port of call for queries on species names in Australia is the [National Species List](https://biodiversity.org.au/nsl/), which is maintained by the Australian Government’s Australian Biological Resources Study. If the list itself cannot help, then a query to the ABRS will often quickly resolve an issue.
